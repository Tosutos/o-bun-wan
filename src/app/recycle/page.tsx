...
export default function RecyclePage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-black">실시간 카메라 탐지 (분리수거)</h1>
      ...
      {!captured && (
        <div className="card space-y-3">
          ...
          />
          <div className="text-xs text-gray-500">카메라 중앙의 ROI 영역에 물체를 맞춰주세요. (신뢰도 70% 이상 시 자동 캡처)</div>
        </div>
      )}
        ...
        <div className="card space-y-3">
          {captured && (
            <div>
              ...
              <div className="mt-2 p-2 rounded border bg-orange-50 text-orange-800 text-sm">
                감지된 물체: <strong>{captured.label}</strong> → 분류: <strong>{result.category}</strong> (신뢰도 {(result.confidence * 100).toFixed(0)}%)
              </div>
            </div>
          )}
          <div className="text-lg font-semibold">분류: {result.category.toUpperCase()}</div>
